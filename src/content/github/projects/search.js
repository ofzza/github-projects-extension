...
export default function bootstrap () { initSearchUI(); }  
...
// ============================================================================================
{
  ...
  initSearchUI = function () {
    ...
    searchBarEl.placeholder = 'Filter cards (GitHub Projects Plus plugin)';
    ...
    if (nativeSearchEl.length) {
      ...
      `;
      $(searchTypeToggleButton).attr('title', 'Filter cards (GitHub Projects Plus plugin)');
      $(searchTypeToggleButton).click(() => { toggleSearch(searchBarContainerEl, searchTypeToggleButton, nativeSearchEl); });
      searchContainerEl.prepend(searchTypeToggleButton);
      
      // Switch to native
      toggleSearch(searchBarContainerEl, searchTypeToggleButton, nativeSearchEl, true);
  ...
  /**
   * Toggles between GitHub native and custom search
   */
  function toggleSearch (searchBarContainerEl, searchTypeToggleButton, nativeSearchEl, initialize) {
    // Select what to toggle to
    let stored = window.localStorage.getItem('searchTypeNative'),
        isNative = (typeof stored !== 'undefined' ? (stored && stored !== 'false') : (nativeSearchEl.style.display == 'block'));
    // Check which filter visible
    if (initialize ? isNative : !isNative) {
      // Show native
    ...
    if (!initialize) {
      window.localStorage.setItem('searchTypeNative', !isNative);
    }
  }
